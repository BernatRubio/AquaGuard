...
struct DecompressionController {
    static func updateSafetyStop(for session: DiveSession) {
        ...
        let measurementDepth: Measurement<UnitLength> = .init(value: UnitConverter.barToMeterSeaWater(gauge), unit: .meters)
    ...
    static func checkSafetyStop(for session: DiveSession) {
        ...
        let safetyStopPressure = session.decoState.currentStop.value
